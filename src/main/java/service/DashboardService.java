...
public class DashboardService {
    ...
    private static final String ID_ENTE = "23";
    ...
    public DashboardData carregarTudo() {
        try (var executor = Executors.newVirtualThreadPerTaskExecutor()) {
            ...
            System.out.println("[Service] Iniciando carregamento...");
            ...
            double despesaTotal = extrair(a06,
                    "DespesaPrimariaTotalExcetoFontesRPPS", "DESPESAS LIQUIDADAS");
            double resultadoPrimario = extrair(a06,
                    "ResultadoPrimarioSemRPPSAcimaDaLinha", "VALOR");

            double receitaTotal = extrair(a06,
                    "ReceitasCorrentesExcetoFontesRPPS", "RECEITAS REALIZADAS (a)");

            double rclTotal = extrair(a06,
                    "ReceitasCorrentesExcetoFontesRPPS", "RECEITAS REALIZADAS (a)");


            System.out.println("[Extrator] Receita:   " + receitaTotal);
            System.out.println("[Extrator] Despesa:   " + despesaTotal);
            System.out.println("[Extrator] Primário:  " + resultadoPrimario);
            System.out.println("[Extrator] RCL:       " + rclTotal);
            ...
            DashboardData data = new DashboardData(
                    ...
                    fIpca.get(),
                    fDolar.get(),
                    "2025 — 2º Bimestre (Até Abril)"
            );

            Thread.ofVirtual().start(() ->
                    snapRepo.salvar(ID_ENTE, ANO, PERIODO, data));
    ...
    private List<RreoItem> fetchComCache(String anexo) {
        try {
            if (rreoRepo.cacheValido(ID_ENTE, ANO, PERIODO, anexo)) {
                System.out.println("[Cache] HIT — " + anexo);
                return rreoRepo.buscarCache(ID_ENTE, ANO, PERIODO, anexo);
            }
            System.out.println("[Cache] MISS — chamando API: " + anexo);
            RreoResponse resp = siconfi.fetchRreo(ANO, PERIODO, anexo, ID_ENTE, "E");
            System.out.println("[API] Resposta " + anexo + " — itens: "
                    + (resp != null && resp.items() != null ? resp.items().size() : "null"));
    ...
    private Map<String, Double> despesaPorFuncao(List<RreoItem> items) {
        List<String> categoriasDespesa = List.of(
        ...
        items.stream()
                .filter(i -> categoriasDespesa.contains(i.co_conta))
    ...
    private Map<String, Double> receitaPorCategoria(List<RreoItem> items) {
        ...
        double outrasTrib    = trib  - icms - ipva - itcd;
        double naoTributaria = total - trib;

        Map<String, Double> map = new java.util.LinkedHashMap<>();
        map.put("ICMS",              icms);
        map.put("IPVA",              ipva);
        map.put("ITCD",              itcd);
        map.put("Outras Tributárias", Math.max(outrasTrib, 0));
        map.put("Não Tributárias",    Math.max(naoTributaria, 0));
        return map;
    }
}